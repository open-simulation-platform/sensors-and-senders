...
{
    m_reals[0] = m_gyro.heading;
    m_strings[1] = m_gyro.sensor_ok;
...
void GyroComponent::step(double step_size) {
    ...
    if(noise_mean != m_gyro.noise.mean()) {
        m_gyro.noise.mean(noise_mean);
    }

    const auto noise_std = m_reals[4];
    if(noise_std != m_gyro.noise.standard_deviation()) {
        m_gyro.noise.standard_deviation(noise_std);
    }

    const auto sensor_error = m_booleans[5];
    m_gyro.sensor_ok = sensor_error ? 'V' : 'A';

    auto heading = m_reals[2];
    ::step(m_gyro, heading, step_size);
